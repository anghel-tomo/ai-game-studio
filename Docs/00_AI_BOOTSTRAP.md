...
1. 人間による最新の明示的な指示
2. `main` ブランチ上の承認済みドキュメント
3. Decision Log に記録された決定
4. Project Status、Project Memory、Session Handover
5. 作業中ブランチの提案・変更
6. 過去のチャット、AIの記憶、未記録の推測

人間の指示によって既存方針を変更する場合は、関連ドキュメントと Decision Log も更新する。
...
1. 対象リポジトリ、ブランチ、現在のタスクを確認する。
2. `Docs/99_AI_CONTEXT.md` で現在地と参照先を確認する。
3. `Docs/00_AI_BOOTSTRAP.md` を読む。
4. Core Documents を番号順に読む。
5. Task-Specific Documents から、現在のタスクに必要な文書を読む。
6. Decision Log と Project Status に矛盾がないか確認する。
...
合意結果または人間の決定は、`Docs/05_DECISION_LOG.md` に記録する。
...
- タスクごと、または一連の文書更新ごとに作業ブランチを使用する。
